...
external.concentration <- get(load("/Volumes/SHAG/GeoTox/data/httk_IVIVE/sensitivity_results_external_conc_RA.RData"))
sensitivity.httk <- get(load("/Volumes/SHAG/GeoTox/data/httk_IVIVE/sensitivity_results_css_RA.RData"))
sensitivity.obesity <- get(load("/Volumes/SHAG/GeoTox/data/httk_IVIVE/sensitivity_results_obesity_RA.RData"))
sensitivity.age <- get(load("/Volumes/SHAG/GeoTox/data/httk_IVIVE/sensitivity_results_age_RA.RData"))
# baseline <- get(load("/Volumes/SHAG/GeoTox/data/final_response_by_county_20211209.RData"))
sensitivity.conc.resp <- get(load("/Volumes/SHAG/GeoTox/data/httk_IVIVE/sensitivity_results_conc_response_RA.RData"))

sensitivity.CR <- NULL
for (x in 1:length(external.concentration)){
  print(x)
  CR <- cbind(external.concentration[[x]]$DA,sensitivity.httk[[x]]$DA,
              sensitivity.obesity[[x]]$DA,sensitivity.age[[x]]$DA,
              sensitivity.conc.resp[[x]]$DA)#,baseline[[x]]$DR)
...
conc.resp.plot <-ggplot(CR.melt, aes(x = value, y = as.factor(Var2), fill = factor(stat(quantile)))) +
  stat_density_ridges(bandwidth = 0.25,
