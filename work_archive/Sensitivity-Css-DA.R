...
run.dr.fun <- function(x){
  ...
  dose.response.RA <- 1 -  (1 - tcplHillVal(log10(invitro.conc.by.county[[x]]),
                                            tp.sim,AC50.sim,slope.sim)) %>% 
    apply(1,prod) 
  ...
  df <- data.frame("DA"= dose.response.DA,"RA" =  dose.response.RA, "HQ" = hazard.quotient,
                   "TP" = tp.val,"AC50" = AC50.val)
  return(df)
}
...
save(final.response.by.county,file = "/Volumes/SHAG/GeoTox/data/httk_IVIVE/sensitivity_results_css_RA.RData")
