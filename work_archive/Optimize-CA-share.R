...
GCA.obj <- function(E,Ci,tp,AC50,slope){
  ...
  if (sum(idx.na)>0){
    ...
    # ECi2 <- inv.tanh(E,params.new[,1],params.new[,2])
    # ECi2 <- linear.inverse(E,params.new[,1],params.new[,2])
    ECi2 <- tcplHillConc2(E,params.new[,1],params.new[,2],1)
    # print(sum(is.na(ECi2)))
    ECi[idx.na] <- ECi2
  }
